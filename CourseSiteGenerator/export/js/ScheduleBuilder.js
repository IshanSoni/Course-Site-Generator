...
function ScheduleItem(sDate, sTitle, sTopic, sLink, sTime) {
    this.date = sDate;
    this.title = sTitle;
    this.topic = sTopic;
    this.link = sLink;
...
function loadJSONData(data) {
    ...
    for (var i = 0; i < data.recitations.length; i++) {
        ...
        var rec = new ScheduleItem(recDate, recData.title, recData.topic, recData.link);
        recitations[i] = rec;
    }
...
function addHolidays() {
  for (var i = 0; i < holidays.length; i++) {
      ...
      var textToAppend = "" + holiday.topic;
      if (holiday.link.valueOf() != "none".valueOf()) {
          textToAppend = "<a href=\"" + holiday.link + "\">"
                        + textToAppend
                        + "</a>";
      }
      textToAppend = "<span class=\"holiday\">"
                     + holiday.title + "<br />"
                     + "</span>"
                     + textToAppend;
...
function addReferences() {
  for (var i = 0; i < references.length; i++) {
      ...
      var textToAppend = "" + reference.topic;
      if (reference.link.valueOf() != "none".valueOf()) {
          textToAppend = "<a href=\"" + reference.link + "\">"
                        + textToAppend
                        + "</a>";
      }
      textToAppend = "<span class=\"reference\">"
                     + reference.title + "<br />"
                     + "</span>"
                     + textToAppend;
      var cell = $("#" + reference.date.month + "_" + reference.date.day);
      cell.append(textToAppend);
  }
}
function addRecitations() {
  for (var i = 0; i < recitations.length; i++) {
      ...
      var textToAppend = "" + recitation.topic;
      if (recitation.link.valueOf() != "none".valueOf()) {
          textToAppend = "<a href=\"" + recitation.link + "\">"
                        + textToAppend
                        + "</a>";
      }
      textToAppend = "<span class=\"recitation\">"
                     + recitation.title + "<br />"
                     + "</span>"
                     + textToAppend;
      var cell = $("#" + recitation.date.month + "_" + recitation.date.day);
      cell.append(textToAppend);
  }
}
...
function addHWs() {
  for (var i = 0; i < hws.length; i++) {
      var hw = hws[i];
      var textToAppend = "" + hw.topic;
      if (hw.link.valueOf() != "none".valueOf()) {
          textToAppend = "<a href=\"" + hw.link + "\">"
                        + textToAppend
                        + "</a>";
      }
      textToAppend = "<span class=\"hw\">"
                     + hw.title + "<br />"
                     + "</span>"
                     + textToAppend;
