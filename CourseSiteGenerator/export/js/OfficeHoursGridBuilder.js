...
function addUndergradTAs(data) {
    ...
    var utas = $("#undergrad_tas");
    utas.append("<p>Undergrad TAs:</p>");
    var utasPerRow = 4;
    var unumTAs = data.undergrad_tas.length;
    for (var i = 0; i < data.undergrad_tas.length; ) {
        var text = "";
        text = "<tr>";
        for (var j = 0; j < utasPerRow; j++) {
            text += buildTACell(i, unumTAs, data.undergrad_tas[i]);
            i++;
        }
        text += "</tr>";
        utas.append(text);
